...
function getAllSubstringsOfLength(string, length) {
  const numberOfSubstrings = string.length - length + 1;
  const substrings = new Set();
  for (let i = 0; i < numberOfSubstrings; i++) {
    substrings.add(string.substr(i, length));
  }
...
function getDeletionCountToAlternateStringWithSubstring(string, substring) { //memo it??. nah using set for substring gen.
  ...
  for (let i = 0; i < string.length; i++) {
    ...
    // console.log(currentSubstring, substring)
...
// console.log(alternatingCharacters(['AAAA','BBBBBB']));
